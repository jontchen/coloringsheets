...
const STYLE_PROMPTS: Record<string, string> = {
  simple:
    "Draw it with very bold, thick black outlines and large, simple shapes that are easy for a young child to color in. Keep the detail minimal — no small parts, no intricate patterns. Think of a coloring book for ages 3-5.",
  cartoon:
  ...
  mandala:
    "Arrange it as a circular mandala pattern with perfect radial symmetry. Use concentric rings of intricate repeating geometric and floral motifs radiating from a center point. The entire design should be contained in a circle.",
  zentangle:
...
export function buildPrompt(config: SheetConfig): string {
  ...
  return [
    `Create a black and white coloring page of ${subject}.`,
    styleInstruction,
    ...
    style?.id === "simple"
      ? ""
      : "Fill the entire page with the design — no large empty margins.",
  ]
    .filter(Boolean)
    .join(" ");
}
